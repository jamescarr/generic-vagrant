...
SCRIPT_LOCATION = "/vagrant/generic-vagrant/scripts"
...
Vagrant.configure(2) do |config|
  ...
  create_node_definitions(nodes_def).each do |node|
    ...
    config.vm.define node[:hostname] do |n|
      ...
      n.vm.provision "shell", inline: "/bin/bash #{SCRIPT_LOCATION}/install_ansible.sh"
...
def update_ansible_galaxy(n)
  if ['up', 'provision', 'reload'].include? ARGV[0]
    if File.exists? THIRD_PARTY_ROLES_FILE
